...
public class RdsService {
    ...
    public List<RdsInstanceInfo> getRdsInstancesViaAssumedRole(String roleArn) {
        ...
        List<RdsInstanceInfo> rdsList = response.dbInstances().stream().map(db->new RdsInstanceInfo( db.dbInstanceArn(),db.dbInstanceIdentifier(), db.engine(), db.dbInstanceStatus(), "us-east-1" )).toList();
